...
let test_1 = ((parse_prefix awkish_grammar accept_all ["$"; "3"; "+"; "5"]) = 
              (  Some
   ([(Expr, [N Term; N Binop; N Expr]); (Term, [N Lvalue]);
     (Lvalue, [T "$"; N Expr]); (Expr, [N Term]); (Term, [N Num]);
     (Num, [T "3"]); (Binop, [T "+"]); (Expr, [N Term]); (Term, [N Num]);
     (Num, [T "5"])],
    [])))

type cool_nonterminals = 
  | Art | Noun | Verb | Adv | Sent | Conj | Adj;;


let cool_sentence_grammar = 
  (Sent,
    function
      | Sent -> 
        [[N Art; N Adj; N Noun; N Verb; N Adv; N Art; N Noun];
        [N Art; N Noun; N Verb]]
      |   Art ->
        [[T "The"];
        [T "the"]]
...
let test_2 = ((parse_prefix cool_sentence_grammar accept_all ["The"; "stealthy"; "ninja"; "snuck"; "through"; "the"; "pagoda"])
          = (Some  ([(Sent, [N Art; N Adj; N Noun; N Verb; N Adv; N Art; N Noun]);
     (Art, [T "The"]); (Adj, [T "stealthy"]); (Noun, [T "ninja"]);
     (Verb, [T "snuck"]); (Adv, [T "through"]); (Art, [T "the"]);
     (Noun, [T "pagoda"])],
    [])))
